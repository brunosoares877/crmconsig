...
const DEFAULT_PRODUCTS = [
  { id: "p1", name: "CARTÃO INSS", code: "" },
  { id: "p2", name: "CREDITO CLT", code: "" },
  { id: "p3", name: "CREDITO FGTS", code: "" },
  { id: "p4", name: "CREDITO INSS", code: "" },
  { id: "p5", name: "CREDITO INSS BPC/LOAS", code: "" },
  { id: "p6", name: "CREDITO PESSOAL", code: "" },
  { id: "p7", name: "CREDITO PIX/CARTAO", code: "" },
  { id: "p8", name: "PORTABILIDADE INSS", code: "" },
  { id: "p9", name: "REFINANCIAMENTO", code: "" }
];
...
const LeadsConfig = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      configuredProducts.forEach((configProduct: Product) => {
        const exists = allProducts.find(p => p.name.toLowerCase() === configProduct.name.toLowerCase());
        if (!exists) {
  ...
  const addProduct = async () => {
    ...
    const exists = products.some(p => p.name.toLowerCase() === prodName.toLowerCase());
    ...
    const userProducts = updatedProducts.filter(p => p.id.startsWith('user-'));
    try {
  ...
  const editBank = async (id: string, newName: string, newCode: string) => {
    ...
    if (!oldBank) return;
    const oldBankValue = oldBank.code || oldBank.name;
    const newBankValue = newCode || newName;
    ...
    if (oldBankValue !== newBankValue) {
      if (window.confirm(`Você alterou os dados do banco "${oldBank.name}". Deseja atualizar este banco em todos os leads antigos que usavam a identificação "${oldBankValue}" para a nova identificação "${newBankValue}"?`)) {
        try {
          const { data: userData } = await supabase.auth.getUser();
          if (userData?.user) {
            const { error } = await supabase.from('leads')
              .update({ bank: newBankValue })
              .eq('bank', oldBankValue)
              .eq('user_id', userData.user.id);
            if (error) throw error;
            toast.success("Leads antigos atualizados com sucesso!");
          }
        } catch(err) {
          logger.error('Erro ao atualizar leads antigos:', err);
          toast.error("Erro ao atualizar os leads antigos.");
        }
      }
    }
  };

  const editProduct = async (id: string, newName: string, newCode: string) => {
    ...
    if (oldName !== newName) {
      if (window.confirm(`Você alterou o nome de "${oldName}" para "${newName}". Deseja atualizar esse produto em todos os leads antigos E nas tabelas de comissões que usavam o nome anterior?`)) {
        try {
          const { data: userData } = await supabase.auth.getUser();
          if (userData?.user) {
            ...
            const { error: ratesError } = await supabase.from('commission_rates')
              .update({ product: newName })
              .eq('product', oldName)
              .eq('user_id', userData.user.id);
            if (ratesError && ratesError.code !== '42P01') logger.warn("Tabela commission_rates não atualizada", ratesError);

            const { error: tiersError } = await supabase.from('commission_tiers')
              .update({ product: newName })
              .eq('product', oldName)
              .eq('user_id', userData.user.id);
            if (tiersError && tiersError.code !== '42P01') logger.warn("Tabela commission_tiers não atualizada", tiersError);

            toast.success("Leads e tabelas de comissões atualizados com o novo produto!");
          }
        } catch(err) {
          logger.error('Erro ao atualizar leads antigos:', err);
          toast.error("Erro ao atualizar os leads antigos.");
  ...
  const editBenefit = async (id: number, newDescription: string, newCode: string) => {
    const oldBenefit = benefitTypes.find(b => b.id === id);
    if (!oldBenefit) return;
    const oldDesc = oldBenefit.description;
    ...
    if (oldDesc !== newDescription) {
      if (window.confirm(`Você alterou o benefício de "${oldDesc}" para "${newDescription}". Deseja atualizar esse benefício em todos os leads antigos que usavam a descrição anterior?`)) {
        try {
          const { data: userData } = await supabase.auth.getUser();
          if (userData?.user) {
            const { error } = await supabase.from('leads')
              .update({ benefit_type: newDescription })
              .eq('benefit_type', oldDesc)
              .eq('user_id', userData.user.id);
            if (error) throw error;
            toast.success("Leads antigos atualizados com sucesso!");
          }
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                {products.map((product) => (
                  <div key={product.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-blue-50 hover:shadow-sm transition-colors duration-200 cursor-pointer">
                    {editingProduct === product.id ? (
                      <div className="flex-1 flex gap-2 items-center">
                        <Input
                          ...
                          className="w-1/2"
                        />
                        <Button size="sm" onClick={() => { editProduct(product.id, newProductName, ""); setEditingProduct(null); setNewProductName(""); }}>Salvar</Button>
                        <Button size="sm" variant="outline" onClick={() => { setEditingProduct(null); setNewProductName(""); }}>Cancelar</Button>
                      </div>
                    ) : (
                      <div>
                    ...
                    <div className="flex items-center gap-2">
                      {editingProduct !== product.id && (
                        <Button size="sm" variant="outline" onClick={() => { setEditingProduct(product.id); setNewProductName(product.name); }}>
                          <Edit className="h-4 w-4" />
                        </Button>
                      )}
                      <AlertDialog>
