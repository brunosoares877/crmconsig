...
const CommissionSettings = () => {
  ...
  const fetchCommissionData = async () => {
    try {
      ...
      const processedRates = (ratesData || []).map((rate) => ({
        ...rate,
        commission_type: (rate as { commission_type?: string }).commission_type || 'percentage',
        fixed_value: (rate as { fixed_value?: number }).fixed_value || null
      })) as CommissionRate[];

      const processedTiers = (tiersData || []).map((tier) => ({
        ...tier,
        commission_type: (tier as { commission_type?: string }).commission_type || 'percentage',
        fixed_value: (tier as { fixed_value?: number }).fixed_value || null
      })) as CommissionTier[];
      ...
      logger.error("Error fetching commission data", err);
  ...
  const handleRateActivation = async (id: string, active: boolean) => {
    try {
      ...
      const err = error as PostgrestError;
      logger.error("Error updating rate status", err);
  ...
  const handleTierActivation = async (id: string, active: boolean) => {
    try {
      ...
      const err = error as PostgrestError;
      logger.error("Error updating tier status", err);
  ...
  const handleDeleteTier = async (id: string) => {
    try {
      ...
      const err = error as PostgrestError;
      logger.error("Error deleting tier", err);
