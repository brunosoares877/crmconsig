...
const Leads = () => {
  ...
  const { banks: configBanks } = useBanks();

  // Memoized lists for dropdowns based strictly on configuration
  const banks = Array.from(new Set(configBanks.map(b => b.code))).sort();
  ...
  const fetchFilterOptions = async () => {
    try {
      ...
      // Não buscamos mais produtos e bancos do DB (legado), agora usamos estritamente os hooks de configuração
              ...
              <Card className="p-4 mb-6">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full">
                    <AddLeadButton onLeadAdded={() => setRefreshTrigger(prev => prev + 1)} />
      ...
      <div className="space-y-6">
        ...
        <Card className="p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            <div className="space-y-2">
              <label className="text-sm font-medium">Data de cadastro (De)</label>
              <Input
                type="date"
                value={dateFrom}
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">Data de cadastro (Até)</label>
              <Input
                type="date"
                value={dateTo}
          ...
          {activeFiltersCount > 0 && (
            <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t">
              ...
              {dateFrom && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  De: {new Date(dateFrom + 'T00:00:00').toLocaleDateString('pt-BR')}
                  <button onClick={() => setDateFrom("")} className="ml-1 hover:bg-gray-300 rounded-full p-0.5">
                    <X className="h-3 w-3" />
                  </button>
                </Badge>
              )}
              {dateTo && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  Até: {new Date(dateTo + 'T00:00:00').toLocaleDateString('pt-BR')}
                  <button onClick={() => setDateTo("")} className="ml-1 hover:bg-gray-300 rounded-full p-0.5">
                    <X className="h-3 w-3" />
                  </button>
                </Badge>
              )}
