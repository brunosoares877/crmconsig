...
export const BankSelect: React.FC<BankSelectProps> = ({
  ...
  const selectedBank = banks.find((bank) => bank.code === value || bank.name === value);

  // Permitir todos os bancos, mesmo sem código
                  ...
                  {validBanks.map((bank) => (
                    ...
                    >
                      <Check
                        className={cn(
                          "mr-2 h-4 w-4",
                          (value === bank.code || value === bank.name) ? "opacity-100" : "opacity-0"
                        )}
                      />
